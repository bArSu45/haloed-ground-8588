...
export default function MensPage() {
  ...
  const getMensData = (sortBy, filter) => {
    axios.get(`http://localhost:8080/womensdata?_sort=discount_price_box&_order=${sortBy}&brand_namez=${filter}`)
      .then((res) => {
  ...
  const getWholeMensData = (sortBy) => {
    axios.get(`http://localhost:8080/womensdata?_sort=discount_price_box&_order=${sortBy}`)
      .then((res) => {
        setMensData(res.data)
